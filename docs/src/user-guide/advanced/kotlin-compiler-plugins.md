...
JetBrains provides a handful of first-party compiler plugins, and some of them are supported in Kargo as first-class
...
??? question "Why do I have to find the plugin ID myself?"
    ...
    Ideally, compiler plugin authors would write a Kargo plugin that wraps their compiler plugin, so they can do this 
    configuration for you (and also provide typed options in their plugin settings).

    At the moment, they can't publish plugins because Kargo doesn't support plugin publication yet, so this low-level
...
version, and thus find the one it needs to use instead of the one used by the CLI (e.g. `./kargo build`).
